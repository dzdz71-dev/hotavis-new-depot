...
export const Route = createFileRoute("/accept-invitation/$token")({
  head: () => ({
    meta: [{ title: "Invitation — Hotavis" }, { name: "robots", content: "noindex" }],
  }),
...
function AcceptInvitation() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const r = await accept({ data: { token, password, full_name: fullName, phone } });
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-muted-foreground mt-2">
            Demandez à l'admin de vous envoyer un nouveau lien.
          </p>
        </div>
      </div>
    );
      ...
      >
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Créez votre mot de passe pour rejoindre l'équipe Hotavis.
          </p>
        </div>
        <div>
          ...
          <input
            value={data.email}
            disabled
            className="mt-1 w-full rounded-lg border border-border bg-muted px-3 py-2 text-muted-foreground"
          />
        </div>
        <div>
