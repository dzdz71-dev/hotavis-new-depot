...
export const getInvitation = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    const { data: inv, error } = await supabaseAdmin
      .from("agent_invitations").select("*").eq("token", data.token).maybeSingle();
...
export const acceptInvitation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await supabaseAdmin.from("user_roles").insert({ user_id: userId, role: "agent" }).then((r) => {
      if (r.error && !r.error.message.includes("duplicate")) throw new Error(r.error.message);
    });

    // Marque l'invitation comme acceptée
    await supabaseAdmin.from("agent_invitations").update({ accepted_at: new Date().toISOString() }).eq("token", data.token);
