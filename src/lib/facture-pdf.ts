...
/**
 * Génère et télécharge un PDF de facture pour une commande.
 * Utilise jsPDF pour dessiner un template propre avec le branding Hotavis.
 */
export function generateFacturePDF(data: FactureData): void {
  ...
  const factureNum = `FAC-${data.commandeId.slice(0, 8).toUpperCase()}`;
  doc.setFontSize(14);
  doc.setFont("helvetica", "bold");
  ...
  doc.text("Montant", pageWidth - margin - 3, tableY + 5.5, { align: "right" });
